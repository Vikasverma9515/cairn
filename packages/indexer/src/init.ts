...
export function runInit(dir: string, options: RunInitOptions = {}): InitResult {
  ...
  if (result.framework === "next-app-router") {
    ...
    result.nextSteps.push(
      "1. cp .cairn/.env.example .env and fill in your key(s).",
      "2. npm install express cors dotenv @cairnvibe/sdk @cairnvibe/core",
...
//
// This runs as its OWN process, separate from your app's real backend (if
// it has one) and on its own port — that's deliberate, not a shortcut: it
// means this works identically whether your app has an Express server, a
// different language's backend entirely, or no backend at all (a static
// site). CORS is on because of that same separation — the page serving
// <cairn-widget> is almost always a different origin (a different dev
// port, or a different domain in production) than this server.
...
if (process.env.DEEPGRAM_API_KEY) {
  ...
  const { createTranscribeHandler } = require("@cairnvibe/sdk/transcribe-server");
  const { Readable } = require("node:stream");
  const speak = createSpeakHandler({ apiKey: process.env.DEEPGRAM_API_KEY });
  const transcribe = createTranscribeHandler({ apiKey: process.env.DEEPGRAM_API_KEY });
  ...
  // Raw audio bytes, not JSON — matches how <cairn-widget>'s push-to-talk
  // mic posts each chunk (see web-component.ts's transcribeSoFar), so no
  // express.json() body-parsing applies here; read the raw buffer instead.
  app.post("/api/copilot/transcribe", express.raw({ type: "*/*", limit: "25mb" }), async (req, res) => {
    const contentType = req.headers["content-type"] || "audio/webm";
    const result = await transcribe(req.body, contentType);
    res.status(result.status).json(result.body);
  });
}
