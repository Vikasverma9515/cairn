...
//  1. Tiles may carry `content` (any React node) instead of an `image`, so tiles can be live, selectable text.
//  2. The animation loop pauses while the wall is scrolled out of view.
//  3. useLayoutEffect falls back to useEffect during server rendering.
const useIsoLayoutEffect = typeof window !== 'undefined' ? useLayoutEffect : useEffect;

/** @type {Array<{ image?: string, title?: string, href?: string, content?: import('react').ReactNode }>} */
...
const DriftWall = ({
  ...
  // Pause the animation loop while the wall is off-screen.
  useEffect(() => {
  ...
  useEffect(() => {
    // Always place the wall once, even when the loop is paused.
  ...
  const renderTile = (item, id, colIndex) => {
    const inner = (
      <span className="drift-wall__inner">
        {item.content ? (
          <span className="drift-wall__content">{item.content}</span>
        ) : (
