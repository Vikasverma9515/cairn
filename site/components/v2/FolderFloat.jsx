...
//  1. useLayoutEffect falls back to useEffect during server rendering.
//  2. JSDoc prop types, so TypeScript callers get accurate types.
//  3. The physics time step is capped at 16.6ms, just under the 16.667ms maximum matter-js recommends (a slow frame no longer logs a warning).
 ...
 *   folderColor?: string, frontColor?: string, paperColor?: string, itemColor?: string, itemTextColor?: string, labelColor?: string,
 *   width?: number, height?: number, radius?: number, spread?: number, lift?: number, tilt?: number,
 *   flapAngle?: number, restAngle?: number, openDuration?: number, stagger?: number, bounce?: number, className?: string
 * }} props
 */
export default function FolderFloat({
  ...
  const startPhysics = useCallback(() => {
    ...
    const tick = now => {
      ...
      if (!s.engine) return;
      const dt = s.last ? Math.min(16.6, now - s.last) : 16;
      s.last = now;
